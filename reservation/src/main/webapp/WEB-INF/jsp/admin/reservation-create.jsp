...
<div class="container">
    ...
    <!-- 예약 정보 -->
    <form action="<c:url value='/admin/reservation/save' />" method="post">
        ...
        <div class="form-row">
		    ...
		    <input type="text" id="memberName" name="memberName" required readonly placeholder="사용자 검색 버튼을 클릭하세요" />
		    <button type="button" onclick="openMemberSearchPopup()">사용자 검색</button>
        </div>
        <div class="form-row">
            <label>전화번호</label>
            <input type="text" id="memberPhoneNumber" name="memberPhoneNumber" required readonly />
        </div>
        <div class="form-row">
            <label>예약 구분</label>
    ...
    <button onclick="location.href='<c:url value='/admin/scheduleDetail?schedulePk=${schedule.schedulePk}' />'" class="btn">이전</button>
</div>
<script>
	//공통: 한 줄 생성하는 함수
    ...
    function openMemberSearchPopup() {
        const popupUrl = "<c:url value='/admin/member/searchPopup' />";
        window.open(popupUrl, '사용자 검색', 'width=800,height=600,scrollbars=yes');
    }

 	// 필드에 적용
    function setMember(memberPk, memberName, memberPhoneNumber) {
        document.getElementById('memberPk').value = memberPk;
        document.getElementById('memberName').value = memberName;
        document.getElementById('memberPhoneNumber').value = memberPhoneNumber;
    }
    ...
    document.getElementById("submitReservation").addEventListener("click", function (event) {
        ...
        if (!memberName) {
            alert("대표 예약자를 선택해주세요!");
            event.preventDefault(); // POST 요청 차단
            return;
        }

        if (!memberPhoneNumber) {
            alert("대표 예약자의 전화번호를 입력해주세요!");
            event.preventDefault(); // POST 요청 차단
            return;
        }
    });
